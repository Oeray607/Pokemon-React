...
const TypeFire = () => {
  ...
  useEffect(() => {
    fetch('https://api.pokemontcg.io/v2/cards')
      ...
      .then((data) => {
        const uniqueFirePokemons = data.data.reduce((acc, pokemon) => {
          const isFireType = pokemon.types && pokemon.types.includes('Fire');
          if (isFireType && !acc.some((p) => p.name === pokemon.name)) {
            acc.push(pokemon);
          }
          return acc;
        }, []);
        setPokemonList(uniqueFirePokemons);
        setFilteredPokemonList(uniqueFirePokemons); // Başlangıçta tüm kartlar gösteriliyor
      })
  ...
  if (!pokemonList.length) {
    return (
      <div style={{ position: 'relative', height: '100vh', width: '100vw' }}>
        <img
          src="https://media4.giphy.com/media/loLOM9vO0g8TwsVkFu/giphy.gif?cid=6c09b952cathh7y86ou8ds4jzf9rw95ub6m0li3ogg1ie1mk&ep=v1_internal_gif_by_id&rid=giphy.gif&ct=g"
