...
const TypeDragon = () => {
  ...
  useEffect(() => { // API
    fetch('https://api.pokemontcg.io/v2/cards')
      ...
      .then((data) => {
        const uniqueDragonPokemons = data.data.reduce((acc, pokemon) => {
          const isDragonType = pokemon.types && pokemon.types.includes('Dragon'); // TUR KONTROLU
          if (isDragonType && !acc.some((p) => p.name === pokemon.name)) {
            acc.push(pokemon); // AYNI ISIMLI TEK BIR POKE
          }
          return acc;
        }, []);
        setPokemonList(uniqueDragonPokemons); // FILTER POKE
        setFilteredPokemonList(uniqueDragonPokemons); // Başlangıçta tüm kartlar gösteriliyor
      })
  ...
  if (!pokemonList.length) {
    return (
      <div style={{ position: 'relative', height: '100vh', width: '100vw' }}>
        <img
          id='Dragonyükleme'
          src="https://i.pinimg.com/originals/df/9a/5e/df9a5e6339cbed6246cb2389f84c93d8.gif"
