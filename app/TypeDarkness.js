...
const TypeDarkness = () => {
  ...
  useEffect(() => { // API
    fetch('https://api.pokemontcg.io/v2/cards')
      ...
      .then((data) => {
        const uniqueDarknessPokemons = data.data.reduce((acc, pokemon) => {
          const isDarknessType = pokemon.types && pokemon.types.includes('Darkness'); // TUR KONTROLU
          if (isDarknessType && !acc.some((p) => p.name === pokemon.name)) {
            acc.push(pokemon); // AYNI ISIMLI TEK BIR POKE
          }
          return acc;
        }, []);
        setPokemonList(uniqueDarknessPokemons); // FILTER POKE
        setFilteredPokemonList(uniqueDarknessPokemons); // Başlangıçta tüm kartlar gösteriliyor
      })
  ...
  if (!pokemonList.length) {
    return (
      <div style={{ position: 'relative', height: '100vh', width: '100vw' }}>
        <img
          id='Darknessyükleme'
          src="https://i.pinimg.com/originals/c7/7c/43/c77c43a23a9a9282de19154b4a2d11f2.gif"
